...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadPreviewDataFromLocalStorage(); // Tải dữ liệu preview
...
function updateCurrentDateAndWeek() {
    ...
    // CHỈ KHI USER ĐANG Ở TAB CHÍNH (không phải preview tab)
    const scheduleTab = document.getElementById('schedule-tab');
    const isOnScheduleTab = scheduleTab && scheduleTab.classList.contains('active');
...
// ===== PREVIEW REGISTRATION FUNCTIONALITY =====

// Hàm lưu dữ liệu preview vào localStorage
function savePreviewDataToLocalStorage() {
    ...
    localStorage.setItem('studentPreviewData', JSON.stringify(savedData));
    console.log('✅ Đã lưu preview data vào localStorage');
}

// Hàm tải dữ liệu preview từ localStorage
function loadPreviewDataFromLocalStorage() {
    ...
    if (savedData) {
        try {
            ...
            if (now - lastUpdated > sixMonthsInMs) {
                ...
                console.log('🗑️ Xóa preview data cũ');
                return;
            }
            
            // Khôi phục dữ liệu input
            if (data.registrationInput) {
                document.getElementById('registration-input').value = data.registrationInput;
                
                // Tự động tạo preview nếu có dữ liệu
                const scheduleData = parseRegistrationData(data.registrationInput);
                if (scheduleData.length > 0) {
                    displayPreviewSchedule(scheduleData);
                    console.log(`✅ Đã khôi phục ${scheduleData.length} môn học từ localStorage`);
                }
            }
        } catch (e) {
            console.error('❌ Lỗi khi đọc preview data:', e);
            localStorage.removeItem('studentPreviewData');
        }
    }
}

// Màu sắc cho preview schedule
const subjectColors = [
    { light: '#FFE4E1', dark: '#FF69B4', accent: '#FF1493' }, // Hồng
    { light: '#E6F3FF', dark: '#4169E1', accent: '#0000FF' }, // Xanh dương
    { light: '#FFFACD', dark: '#FFD700', accent: '#FFA500' }, // Vàng
    { light: '#F0FFF0', dark: '#32CD32', accent: '#008000' }, // Xanh lá
    { light: '#F5F0FF', dark: '#9370DB', accent: '#8A2BE2' }, // Tím
    { light: '#FFE4B5', dark: '#FF8C00', accent: '#FF4500' }, // Cam
    { light: '#F5F5F5', dark: '#A9A9A9', accent: '#696969' }, // Xám
    { light: '#E0E6FF', dark: '#5366FF', accent: '#3B4CCA' }, // Xanh dương đậm
    { light: '#FFE4FF', dark: '#FF63FF', accent: '#FF00FF' }, // Hồng đậm
    { light: '#E4FFE6', dark: '#63FF84', accent: '#00FF32' }  // Xanh lá đậm
];

// Hàm phân tích dữ liệu đăng ký môn theo format mới
function parseRegistrationData(input) {
    const scheduleData = [];
    const lines = input.trim().split('\n');
    ...
    while (i < lines.length) {
        ...
        // Tìm dòng môn học
        const subjectMatch = line.match(/^(\d+)([A-Z0-9]+)\s*-\s*(.+?)(\d+\.\d+)$/);
        if (subjectMatch) {
            let subjectName = subjectMatch[3].trim();
            const credits = parseFloat(subjectMatch[4]);
            
            // Loại bỏ các ký tự dính liền trước credits
            subjectName = subjectName.replace(/\s*-\s*[a-z]*$/, '').trim();
            ...
            // Tìm và xử lý tất cả dòng time data cho môn này
            i++; // Di chuyển đến dòng tiếp theo
            
            while (i < lines.length) {
                const nextLine = lines[i].trim();
                // console.log(`  Checking line ${i}: "${nextLine}"`);
                
                // Nếu gặp môn học mới thì dừng
                if (nextLine.match(/^(\d+)([A-Z0-9]+)\s*-\s*(.+?)(\d+\.\d+)$/)) {
                    // console.log('  📌 Found next subject, stopping');
                    break;
                }
                
                // Bỏ qua headers và group info
                if (nextLine.includes('Nhóm lớp') || 
                    nextLine.includes('DK/ Sĩ số') || 
                    nextLine === '') {
                    // console.log('  📝 Skipping:', nextLine);
                    i++;
                    continue;
                }
                
                // Bỏ qua header line riêng
                if (nextLine.includes('Thứ') && nextLine.includes('Tiết') && nextLine.includes('Phòng')) {
                    // console.log('  📝 Skipping header line:', nextLine);
                    i++;
                    continue;
                }
                
                // Bỏ qua group info line riêng  
                if (nextLine.match(/^[A-Z0-9_]+(\s+\d+\/\d+|\t)/)) {
                    // console.log('  📝 Skipping group line:', nextLine);
                    i++;
                    continue;
                }
                
                // Xử lý dòng time data - normalize Unicode để xử lý combining characters
                const normalizedLine = nextLine.normalize('NFC');
                const isTimeData = normalizedLine.startsWith('Thứ') || 
                                 normalizedLine.startsWith('Chưa biết') ||
                                 nextLine.includes('Thứ 2') || nextLine.includes('Thứ 3') ||
                                 nextLine.includes('Thứ 4') || nextLine.includes('Thứ 5') ||
                                 nextLine.includes('Thứ 6') || nextLine.includes('Thứ 7') ||
                                 nextLine.includes('Chưa biết');
                
                if (isTimeData && !nextLine.includes('Tiết')) {
                    // console.log('  ⏰ Processing time data:', nextLine);
                    ...
                    if (parts.length < 6) {
                        parts = originalLine.split(/\s{2,}/);
                    }
                    
                    if (parts.length >= 6) {
                        const dayStr = parts[0].trim();
                        const periodsStr = parts[1].trim();
                        const room = parts[2].trim();
                        const weeksStr = parts[5].trim();
                        
                        // console.log('  📊 Extracted:', {dayStr, periodsStr, room, weeksStr});
                        
                        // Xử lý ngày trong tuần
                        let day = 0;
                        if (dayStr.includes('2')) day = 2;
                        else if (dayStr.includes('3')) day = 3;
                        else if (dayStr.includes('4')) day = 4;
                        else if (dayStr.includes('5')) day = 5;
                        else if (dayStr.includes('6')) day = 6;
                        else if (dayStr.includes('7')) day = 7;
                        
                        if (day > 0) {
                            // Xử lý chuỗi tiết học
                            const periods = [];
                            const periodParts = periodsStr.split(/\s+/);
                            periodParts.forEach((part) => {
                                if (part !== '-' && !isNaN(parseInt(part))) {
                                    periods.push(parseInt(part));
                                }
                            });
                            ...
                            if (periods.length > 0) {
                                ...
                                const weeks = parseRegistrationWeeks(weeksStr);
                            ...
                            // console.log('  ❌ Skipping unknown day:', dayStr);
                        }
                    }
                }
    ...
    // console.log('Total courses found:', scheduleData.length);
    // console.log('Schedule data:', scheduleData);
    return scheduleData;
}

// Hàm phân tích chuỗi tuần học cho preview registration
function parseRegistrationWeeks(weeksStr) {
    const weeks = [];
    if (!weeksStr) return weeks;
    
    // Chuyển đổi chuỗi dạng "12345678-0123456--------------" thành mảng tuần
    for (let i = 0; i < weeksStr.length; i++) {
        const char = weeksStr[i];
        if (char !== '-' && char !== '0') {
            weeks.push(i + 1); // Tuần bắt đầu từ 1
        }
    }
...
// Hàm khởi tạo grid cho preview schedule (desktop)
function initializePreviewScheduleGrid() {
    const grid = document.getElementById('preview-schedule-grid');
    if (!grid) return;
    ...
    // Tạo 16 * 6 ô cho grid (16 tiết x 6 ngày)
    for (let i = 0; i < 16 * 6; i++) {
        const cell = document.createElement('div');
        cell.className = 'schedule-cell';
        grid.appendChild(cell);
...
// Hàm khởi tạo mobile schedule cho preview
function initializePreviewMobileSchedule() {
    const mobileTimetable = document.getElementById('preview-mobile-timetable');
    if (!mobileTimetable) return;
    
    const days = [2, 3, 4, 5, 6, 7];
    
    days.forEach(day => {
        const dayContainer = mobileTimetable.querySelector(`[data-day="${day}"] .mobile-time-slots`);
        if (dayContainer) {
            dayContainer.innerHTML = '';
            
            // Tạo 16 time slots cho mỗi ngày
            for (let period = 1; period <= 16; period++) {
                ...
                let timeText = '';
                switch(period) {
                    case 1: timeText = '06:00 - 06:50'; break;
                    case 2: timeText = '07:00 - 07:50'; break;
                    case 3: timeText = '08:00 - 08:50'; break;
                    case 4: timeText = '09:00 - 09:50'; break;
                    case 5: timeText = '10:00 - 10:50'; break;
                    case 6: timeText = '11:00 - 11:50'; break;
                    case 7: timeText = '12:00 - 12:50'; break;
                    case 8: timeText = '13:00 - 13:50'; break;
                    case 9: timeText = '14:00 - 14:50'; break;
                    case 10: timeText = '15:00 - 15:50'; break;
                    case 11: timeText = '16:00 - 16:50'; break;
                    case 12: timeText = '17:00 - 17:50'; break;
                    case 13: timeText = '18:00 - 18:50'; break;
                    case 14: timeText = '18:50 - 19:40'; break;
                    case 15: timeText = '19:40 - 20:30'; break;
                    case 16: timeText = '20:30 - 21:10'; break;
                }
                
                timeSlot.innerHTML = `
                    <div class="time-label">Tiết ${period}</div>
                    <div class="time-range">${timeText}</div>
...
// Hàm hiển thị preview schedule
function displayPreviewSchedule(scheduleData) {
    // Không cần selectedWeek vì hiển thị tất cả môn học
    
    // Khởi tạo grid trước khi hiển thị
    initializePreviewScheduleGrid();
    initializePreviewMobileSchedule();
    
    // HIỂN THỊ TẤT CẢ môn học không filter theo tuần (như yêu cầu của user)
    const filteredData = scheduleData; // Không filter theo tuần nữa
    
    // console.log('📅 Displaying all courses without week filter:', filteredData);
    
    // Hiển thị trên desktop
    displayPreviewDesktopSchedule(filteredData);
    
    // Hiển thị trên mobile
    displayPreviewMobileSchedule(filteredData);
    
    // Cập nhật thông tin ngày và tuần
    updatePreviewCurrentDateAndWeek();
}

// Hàm hiển thị preview schedule trên desktop (giống hệt trang tạo thời khóa biểu)
function displayPreviewDesktopSchedule(scheduleData) {
    const grid = document.getElementById('preview-schedule-grid');
    if (!grid) return;
    
    // Xóa tất cả các môn học hiện tại và RESET VỀ TRẠNG THÁI BAN ĐẦU
    ...
    cells.forEach(cell => {
        ...
        // ĐẢM BẢO TẤT CẢ CELLS CÓ VIỀN ĐẦY ĐỦ
        cell.style.borderBottom = '1px solid #e0e0e0';
    ...
    setTimeout(() => {
        scheduleData.forEach((course, index) => {
            ...
            // Tính toán vị trí cell trong grid (giống hệt trang chính)
            const cellIndex = 6 + (startIndex * 6) + dayIndex + 1;
            
            const cell = document.querySelector(`#preview-schedule-grid .schedule-cell:nth-child(${cellIndex})`);
            
            if (cell) {
                // Sử dụng hệ thống màu giống trang chính
                const color = getColorForCourse(course.code);
                cell.style.backgroundColor = color;
                cell.style.gridRow = `span ${duration}`;
                cell.style.borderBottom = '1px solid #e0e0e0';
                ...
                // Format nội dung giống hệt trang chính
                cell.innerHTML = `
                    <div style="height: 100%; opacity: 0; transform: translateY(10px); transition: all 0.3s ease ${index * 0.05}s;">
                        <strong>${course.code}</strong>
                        ${course.name}<br>
                        Phòng: ${course.room}<br>
                        Thời gian: ${course.time || 'N/A'}<br>
                        Nhóm: ${course.group || 'N/A'}
                    </div>
                `;
                
                // Hiệu ứng xuất hiện giống trang chính
                setTimeout(() => {
                    const contentDiv = cell.querySelector('div');
                    if (contentDiv) {
                        contentDiv.style.opacity = '1';
                ...
                // Xóa các cell bị che phủ với hiệu ứng mờ dần (NHƯNG GIỮ VIỀN)
                for (let i = 1; i < duration; i++) {
                    const nextCellIndex = cellIndex + (i * 6);
                    const nextCell = document.querySelector(`#preview-schedule-grid .schedule-cell:nth-child(${nextCellIndex})`);
                    if (nextCell) {
                        nextCell.style.opacity = '0';
                        nextCell.style.pointerEvents = 'none';
                        // ĐẢM BẢO VIỀN VẪN CÓ TRÊN CELLS BỊ CHE PHỦ
                        nextCell.style.borderBottom = '1px solid #e0e0e0';
                        nextCell.style.borderRight = '1px solid #e0e0e0';
                    }
                }
            }
        });
...
// Hàm hiển thị preview schedule trên mobile (giống hệt trang chính)
function displayPreviewMobileSchedule(scheduleData) {
    // Xóa tất cả các lớp học hiện tại
    document.querySelectorAll('#preview-mobile-timetable .mobile-time-slot .content, #preview-mobile-timetable .mobile-time-slot .subject-info').forEach(content => {
        if (content) {
            content.innerHTML = '';
            content.className = content.classList.contains('content') ? 'content' : 'subject-info';
        }
    });
    ...
    // Hiển thị môn học theo ngày và tiết học (giống trang chính)
    for (const day in scheduledByDay) {
        const dayContainer = document.querySelector(`#preview-mobile-timetable [data-day="${day}"]`);
        if (!dayContainer) continue;
        ...
        scheduledByDay[day].forEach((course, index) => {
            ...
            // Thêm thông tin môn học vào tiết bắt đầu
            const timeSlot = dayContainer.querySelector(`[data-period="${startPeriod}"]`);
            if (timeSlot) {
                let content = timeSlot.querySelector('.content');
                if (!content) {
                    content = timeSlot.querySelector('.subject-info');
                }
                
                if (content) {
                    content.classList.add('has-class');
                    
                    // Thêm màu tương ứng cho môn học (giống trang chính)
                    const colorIndex = (index % 7) + 1;
                    content.classList.add(`color${colorIndex}`);
                    timeSlot.classList.add(`color${colorIndex}`);
                    
                    // Format giống hệt trang chính
                    content.innerHTML = `
                        <div class="subject-code">${course.code}</div>
                        <div class="subject-name">${course.name}</div>
                        <div class="room">Phòng: ${course.room}</div>
                    ...
                    // Áp dụng hiệu ứng xuất hiện dần (giống trang chính)
                    content.style.opacity = '0';
                    content.style.transform = 'translateY(10px)';
...
// Hàm cập nhật ngày và tuần hiện tại cho preview
function updatePreviewCurrentDateAndWeek() {
    const now = new Date();
    const currentDateElement = document.getElementById('preview-current-date');
    const currentWeekElement = document.getElementById('preview-current-week');
...
// Event listeners cho preview registration
document.addEventListener('DOMContentLoaded', function() {
    // Event listener cho nút tạo preview schedule
    const generatePreviewBtn = document.getElementById('generate-preview-schedule');
    if (generatePreviewBtn) {
        generatePreviewBtn.addEventListener('click', function() {
            const registrationInput = document.getElementById('registration-input').value;
            
            if (registrationInput.trim() === '') {
                alert('Vui lòng nhập dữ liệu đăng ký môn!');
                return;
            }
            
            const scheduleData = parseRegistrationData(registrationInput);
            ...
            // Hiển thị preview schedule
            displayPreviewSchedule(scheduleData);
            
            // Lưu vào localStorage
            savePreviewDataToLocalStorage();
            
            // Hiển thị thông báo thành công
            alert(`Đã tạo preview thành công cho ${scheduleData.length} môn học!`);
        });
    }
    
    // Thêm nút xóa dữ liệu preview đã lưu
    const previewInputSection = document.querySelector('#preview-registration-tab .input-section');
    if (previewInputSection) {
        const clearPreviewDataButton = document.createElement('button');
        clearPreviewDataButton.textContent = 'Xóa dữ liệu đã lưu';
        clearPreviewDataButton.style.marginLeft = '10px';
        clearPreviewDataButton.style.backgroundColor = '#e74c3c';
        clearPreviewDataButton.style.color = 'white';
        clearPreviewDataButton.style.border = 'none';
        clearPreviewDataButton.style.padding = '8px 15px';
        clearPreviewDataButton.style.borderRadius = '25px';
        clearPreviewDataButton.style.cursor = 'pointer';
        clearPreviewDataButton.addEventListener('click', function() {
            if (confirm('Bạn có chắc chắn muốn xóa dữ liệu preview đã lưu?')) {
                localStorage.removeItem('studentPreviewData');
                ...
                alert('Đã xóa dữ liệu preview!');
            }
        });
        previewInputSection.appendChild(clearPreviewDataButton);
    }
    
    // Không cần event listener cho week select vì đã bỏ phần chọn tuần
    
    // Event listeners cho day selector trên mobile
    const previewMobileTimetable = document.getElementById('preview-mobile-timetable');
    if (previewMobileTimetable) {
        const dayButtons = previewMobileTimetable.querySelectorAll('.day-selector button');
        const mobileDays = previewMobileTimetable.querySelectorAll('.mobile-day');
    ...
    // Khởi tạo preview schedule khi chuyển tab
    const previewTabBtn = document.querySelector('[data-tab="preview-registration-tab"]');
    if (previewTabBtn) {
