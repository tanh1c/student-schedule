...
export async function performCASLogin(username, password) {
    ...
    try {
        logger.info('[CAS] Step 1: Getting login form...');
        ...
        if (!executionMatch || !ltMatch) {
            logger.info('[CAS] Failed to parse login form. HTML length:', html.length);
            return { success: false, error: 'Không thể tải form đăng nhập SSO' };
        }

        const execution = executionMatch[1];
        const lt = ltMatch[1];
        logger.info('[CAS] Got tokens. execution:', maskSensitive(execution));

        // Step 2: Submit login form
        logger.info('[CAS] Step 2: Submitting credentials...');
        ...
        logger.info('[CAS] Final URL after login:', maskUrl(finalUrl));
        ...
        if (hiddenTokenMatch) {
            ...
            logger.info('[AUTH] Found token via hid_Token input! 🎉');
        } else if (localStoreMatch) {
            jwtToken = `Bearer ${localStoreMatch[1]}`;
            logger.info('[AUTH] Found token via localStorage pattern');
        } else if (windowMatch) {
            jwtToken = `Bearer ${windowMatch[1]}`;
            logger.info('[AUTH] Found token via window pattern');
        } else {
            ...
            if (jwtMatch) {
                logger.info('[AUTH] Found raw JWT pattern, trying it...');
                jwtToken = `Bearer ${jwtMatch[0]}`;
            } else {
                logger.info('[AUTH] Could not find token in HTML.');
            }
        }
        ...
        logger.info('[CAS] Cookies from /app path:', maskCookie(cookieString));

        if (!cookieString.includes('SESSION')) {
            logger.info('[WARN] SESSION cookie missing! Trying root path...');
            const rootCookies = await jar.getCookies('https://mybk.hcmut.edu.vn/');
            logger.info('[CAS] Root cookies:', maskCookie(rootCookies.map(c => c.cookieString()).join('; ')));
        }
        ...
        if (infoResponse.ok) {
            ...
            try {
                ...
                if (data.code === "200" || data.code === 200 || !data.code) {
                    ...
                    logger.info('[AUTH] Student info fetched successfully!');
                } else {
                    logger.info(`[AUTH] Login OK but info denied: ${data.code} - ${data.msg}`);
                }
            } catch (e) { logger.info('[AUTH] Response not JSON', e); }

            const authHeader = infoResponse.headers.get('authorization');
            if (authHeader) {
                jwtToken = authHeader;
                logger.info('[AUTH] Found token in API response header');
            }
        } else {
