...
const HonorWallPage = lazy(() => import("./HonorWallPage"));
...
export default function SettingsPage() {
    ...
    const ActiveSettingsContent = useMemo(() => {
        if (activeTab === "security") return SecurityPage;
        if (activeTab === "changelog") return ChangelogPage;
        if (activeTab === "honor") return HonorWallPage;
        return LocalStorageViewer;
    }, [activeTab]);

    const isLazySettingsTab = activeTab !== "storage";
                    ...
                    <Suspense
                        fallback={
                            <div className="flex min-h-[320px] items-center justify-center px-4 py-10">
                                <div className="w-full max-w-sm rounded-2xl border bg-card/80 p-6 text-center shadow-sm backdrop-blur">
                                    <div className="mx-auto mb-3 h-10 w-10 animate-pulse rounded-2xl bg-primary/10" />
                                    <p className="text-sm font-semibold text-foreground">
                                        Đang tải nội dung...
                                    </p>
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        Tab này sẽ hiển thị ngay khi tải xong.
                                    </p>
                                </div>
                            </div>
                        }
